...
<template>
    <div>
        ...
        <div class="flex mb-16">
            <PlayPauseButton
                @click="play.singleNote(selectedNote)"
                :animate-on-play="false"
            ...
            <fieldset class="note-group" role="group">
                ...
                <div class="flex flex-wrap gap-2" aria-labelledby="play-single-note">
                    <UButton
                        v-for="note in simpleSynthNotes"
                        :key="note"
                        class="text-amber-100"
                        :class="['w-10 justify-center', selectedNote === note ? 'liquid-wheel' : '']"
                        size="md"
            ...
            <fieldset class="note-group w-full" role="group">
                ...
                <div class="note-options flex flex-wrap gap-2 mb-4">
                    <div v-for="note in simpleSynthNotes" :key="note" class="addable-note group relative">
                        ...
                        >
                            <UIcon
                                name="mingcute-add-circle-fill"
                                class="size-5 text-primary font-extrabold stroke-8"
                            />
                        </div>
                    </div>
                </div>
                ...
                >
                    <div v-for="(noteItem, index) in noteSequence" :key="index" class="removable-note group relative">
                        <UButton
                            class="text-amber-100 cursor-pointer transition-all duration-150"
                            :class="[
                                'w-10 justify-center',
                                play.currentPlayingIndex.value === index
                                    ? 'liquid-wheel'
                                    : 'group-hover:ring-1 group-hover:ring-gray-700 group-hover:scale-95',
                            ]"
                            size="md"
                            color="primary"
                            @click="removeNoteFromSequence(index)"
                        >
                            {{ noteItem.replace('4', '') }}
                        </UButton>
                        <div
                        ...
                        >
                            <UIcon
                                name="mingcute-minus-circle-fill"
                                class="size-5 text-gray-700 font-extrabold stroke-8"
                            />
                        </div>
                    </div>
                    <UButton
                    ...
                    />
                    <UButton
                        v-if="noteSequence.length > 0"
                        class="-translate-x-6"
                        variant="ghost"
                        size="sm"
                        icon="i-heroicons-x-mark"
                        @click="clearNoteSequence"
